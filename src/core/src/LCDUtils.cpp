#include "core/include/LCDUtils.h"
...
LCDUtils::LCDUtils(tapuino::PlayerActivity& player) : player_(player)
{
    lastTick = millis();
    lastHoldTick = 0;
    fileNameBuf[0] = 0;
...
// {
//     player_.getTask()->showAlertDialog(title, msg, {"OK"}, [](int){});
//     // Title(title);
//     // Status(msg);
//     // for (int i = 0; i < 10; i++)
//     // {
...
// void LCDUtils::ShowProgress(bool motor, uint16_t counter, uint32_t total, uint32_t read)
// {
//     player_.showProgress(total, read);
// }
